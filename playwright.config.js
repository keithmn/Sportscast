...
module.exports = defineConfig({
  ...
  webServer: {
    command: `rm -f "${DB_PATH}" && DATABASE_URL="file:${DB_PATH}" npx prisma migrate deploy && DATABASE_URL="file:${DB_PATH}" node prisma/seed.js && DATABASE_URL="file:${DB_PATH}" SESSION_SECRET=e2e-test-secret-not-used-in-production PORT=${PORT} node server/index.js`,
    url: BASE_URL,
