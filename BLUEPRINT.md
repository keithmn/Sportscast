...
## 35. Image performance pass, and a stale-docs correction (2026-09-13)

The single largest concrete performance risk an earlier audit named: unoptimized images, zero `loading="lazy"`. Addressed the part that's actually low-risk to fix without a design conversation:

- `loading="lazy"` added to every content `<img>` across `public/js/*.js` and the two remaining static ones (`shows.html`, `show.html`) — article covers, crests, player photos, show art. The two always-visible header/footer brand logos (`site.js`) deliberately kept eager (they're small PNGs and always above the fold, not the problem).
- `public/images/shows/the-sportscast.jpg` — the one image actually referenced by a live production code path (confirmed by checking real production API responses, not just grep) — compressed from a raw 3.1MB/3000×4000 phone photo down to ~700KB at 1200px wide (`sips`), visually verified to still look sharp at that size. No other file in `public/images/` is currently referenced by any code path (confirmed the same way): `hero.jpg` backed a homepage hero section removed 2026-08-20; the 5 niche shows' cover art (`shows/the-hydration-break.jpg` etc.) went orphaned when those shows' actual content was deleted (§28) — `public/images/README.md` was describing all of these as live "drop-in slots" that would "appear automatically," which was no longer true. Corrected the README to say so plainly rather than leave it giving false instructions. The image files themselves were left untouched — they're the user's own real photos, not code, and deleting them wasn't asked for.

Not touched: the raw phone/HEIC dump files sitting in `public/images/` root (`20260221_*.jpg`, several `.heic` files, `1000183612.jpg`) — genuinely unreferenced by anything, but real user-provided media, not something to delete without being asked.

## 36. Fixture.homeTeam/awayTeam gain a best-effort Club FK (2026-09-14)

The one architecture gap this codebase had explicitly flagged as too risky to fix casually (§32's schema comment, since corrected): `Fixture.homeTeam`/`awayTeam` were free-text strings, matched to a real `Club` only by eyeballing spelling. Built as additive and fail-closed, not a rewrite:

- **Schema**: `Fixture.homeClubId`/`awayClubId` — nullable FKs to `Club` (`Fixture_homeClubId_fkey`/`_awayClubId_fkey`, `onDelete: SetNull`). `homeTeam`/`awayTeam` strings are untouched and remain the source of truth everywhere, including the sync jobs' `@@unique([competitionId, homeTeam, awayTeam, kickoff])` upsert key — this is a lookup layered on top, never a replacement.
- **Resolver** (`server/lib/clubResolution.js`): ports `public/js/subnav.js`'s existing client-side `fuzzyTeamMatch`, but hardened to fail closed — an exact normalized-name match (case-insensitive, `FC`/`AFC` stripped) resolves if there's exactly one candidate Club in the competition; a substring/containment match only resolves if it's *still* the only candidate. Two same-named or ambiguously-overlapping clubs in one competition resolve to `null`, never a guess. Confirmed against real data: Kenyan Premier League correctly resolved "AFC Leopards"→AFC Leopards and "Gor Mahia"→Gor Mahia FC while correctly leaving "Bandari"/"Murang'a Seal"/"Nairobi United"/etc. unresolved, since those simply have no Club row yet.
- **Wired into every write path** (six total, found by an exhaustive audit before starting — see the session's scoping notes): all 3 sync jobs (`syncLeagues.js`, `syncTheSportsDB.js`, `syncBallDontLie.js`) resolve once per competition per run (not per fixture — avoids N+1 queries) and re-set the FK on every upsert; both admin routes (`POST /:id/fixtures`, `POST /:id/fixtures/bulk`) resolve on create; the edit route (`PUT /fixtures/:fixtureId`) only re-resolves when homeTeam/awayTeam actually changed, otherwise keeps the existing link.
- **Backfill** (`scripts/backfill-fixture-clubs.js`): one-time pass over the 4,589 pre-existing Fixture rows, dry-run by default (`node scripts/backfill-fixture-clubs.js`), `--apply` to write. Applied: 705/705 resolved both sides for the English Premier League (20 real Club rows, clean 1:1 naming), 3/8 home and 3/8 away for Kenyan Premier League (only 6 of its ~14 real clubs have been entered as Club rows so far — everyone else correctly left `null`, not fabricated).
- **Read paths**: `GET /api/fixtures`, `GET /api/fixtures/upcoming`, and `GET /api/competitions/:slug` now include `homeClub`/`awayClub` (`id`/`name`/`slug`/`crestUrl`).
- **Client**: `subnav.js`'s `matchesClub()` now prefers the real FK when a fixture has one, falling back to the old string-fuzzy-match only for fixtures the resolver left unmatched — used by both the club hub page (`club.js`) and the competition page's per-club fixture filter (`subnav.js`). Admin's fixture rows (`admin/js/competitions.js`) show a small "↳ matched: X vs Y" hint so the resolver's behavior is visible, not hidden.
- **Verified**: dry-run and `--apply` output inspected against real data before trusting it; API responses spot-checked directly; a real headless-Chrome session confirmed the club hub page and the admin competitions page both render correctly with the new fields, zero console/page errors; full Playwright e2e suite green before and after.
- **Deliberately still open**: `StandingRow.teamName` is unaffected by this — same free-text-string situation, not revisited here. Most competitions (every football-data.org global league except the one checked) still have zero Club rows at all, so most fixtures will keep resolving to `null` until someone manually enters those clubs — that's the correct, honest behavior, not a bug to chase.

## 37. Match Hub — a real per-fixture detail page (2026-09-15)

Fixture rows (competition page, club page, home page, sport page, the Scores tab) were plain unlinked `<div>`s — no page existed to click through to. Built the smallest page that's actually useful, not a speculative "full match center":

- **Route**: `GET /api/fixtures/:id` (`server/routes/fixtures.js`, registered after the pre-existing `/upcoming` to avoid the `:id` wildcard swallowing it), returns the fixture with `competition` (incl. `sport`) and `homeClub`/`awayClub` (`id`/`name`/`slug`/`crestUrl`).
- **Page** (`public/match.html` + `public/js/match.js`): competition, both sides (club-linked where resolved, plain text otherwise — see §36), kickoff, status, score (shown only once `FINISHED` with a real recorded score — never a fabricated 0-0), a postponed note when `originalKickoff` is set, a provenance line derived from `competition.source`/`externalProvider` (MANUAL/API/SCRAPED), and related coverage.
- **Related coverage**: no direct `Article↔Fixture` link exists in the schema, so this reuses the same heuristic `club.js`'s hub strip already used — articles tagged to either side's Club or the Competition, deduped, sorted by recency, capped at 4. Not new schema, not a guess.
- **Wired in**: `fixtureRowHtml` (`public/js/scores.js`, the one function every fixture-row call site already shares) now wraps its markup in `<a href="/match.html?id=...">` instead of a bare `<div>` — reaches all 4 existing call sites automatically.
- **Deliberately not built**: a canonical Fixture↔Match bridge to the Data Platform (no name field exists on either side to match against — same gap noted for Meet/Club/Fixture search below) and a Player-of-the-Match poll (the `Poll` model is Article-scoped only, not Fixture-scoped).
- **Verified**: real admin-created fixture, reached from the competition page's Scores & Fixtures tab, real data on the page; a missing/bad id shows "not found," not a crash. `e2e/match-hub.spec.js`.

## 38. Server-rendered SEO metadata for entity pages (2026-09-15)

This app is plain `express.static` with no templating layer at all — every `<title>`/description/OG tag on club/player/competition/article/show pages was either a hardcoded generic string or set client-side (too late for a crawler or social-preview bot), and those five pages were hardcoded `noindex`.

- **`server/lib/renderSeoHtml.js`**: reads the real static HTML file fresh per request (`fs.readFileSync`, no caching — this app has no build step, so "fresh per request" is cheap and simplest) and does targeted string replacement of `<title>`, `<meta name="description">`, `og:title`, `og:description`, `og:image`. Only ever *removes* the `noindex` meta tag, never adds one — a page this doesn't recognize keeps its existing (safe) default.
- **`server/routes/seoPages.js`**: 5 routes (club/player/competition/article/show), registered in `server/index.js` *before* `express.static` so a resolving request never reaches the static file at all; `next()` falls through to the plain static file on no slug, not-found, draft article, or inactive show — same client-side handling as before, unchanged. Every handler wrapped in try/catch: a Prisma error here must never crash the whole process. Found the hard way in development — an incorrect field name in the Club query (`city`, which doesn't exist on this app's own `Club` model, confused with the *separate* Data Platform's differently-shaped `Club`) threw inside an async handler with no catch and took the entire server down; fixed alongside adding the catch everywhere, not just at that one call site.
- **Verified**: raw HTML fetch and a real browser page load both checked for a real competition (title, description, `noindex` gone) and a real published article (title, dek); a nonexistent competition still falls through to the generic static page, still `noindex`. `e2e/seo-pages.spec.js`.

## 39. Meet/Club search closes out the Data Platform's search surface (2026-09-15)

The one remaining gap from §32's search work: `Search (Club/Fixture/Meet)` was left for later because Meet/Club needed real investigation, and Fixture is a genuinely different problem, not just an extension of the existing pattern.

- **Meet**: needed almost nothing — `resolveEntities.ts`'s `ResolvableEntityType` gained `'Meet'`, a `case 'Meet':` querying `prisma.meet` (with `sport`), and the search route's `VALID_TYPES`/`ENTITY_TYPE_BY_SEARCH_TYPE` gained `meet`. `apps/web`'s search results page already had the machinery to render a new section; just added `{key: 'meets', ...}`.
- **Club**: the Data Platform's `GET /v1/clubs` API already existed — `apps/web` just had no detail page to link to. Added `apps/web/src/app/clubs/[slug]/page.tsx` (identity, org type, sport, location, website, linked Teams list) and a `?slug=` filter on `GET /v1/clubs` to support the slug→id→detail fetch. Same `resolveEntities.ts`/search-route/results-page additions as Meet.
- **Fixture stays deferred, correctly**: neither this platform's `Fixture` nor Sportscast's own separate `Fixture` model (§36) has a name field — a fixture is only ever identified by its two team names plus a kickoff time, not a single string a search box can match against. Closing this needs a deliberate composite-match design (e.g. "team vs team" query parsing), not a copy of the Meet/Club pattern. Revisit only with that design chosen, not by forcing today's shape onto it.
- **Verified**: `apps/api/src/routes/search.test.ts` covers Meet/Club matching and the "search every type" default; `apps/api/src/routes/clubs.test.ts` covers the new `?slug=` filter.

## 40. Competition seasons — a real relational season model (2026-09-16)

Sportscast had no season concept tied to a Competition at all: `Fixture` and `StandingRow` belonged only to a `competitionId`, so a new season's results could only ever overwrite the previous one — there was no way to keep last season's table around once this season's results started landing, and no way to browse history. Chosen deliberately over a lighter additive-label option: a real relational model, matching how the Data Platform's own architecture already treats seasons, at the cost of touching every fixture/standings write path.

- **Schema** (`prisma/migrations/20260916061757_add_competition_season`): new `CompetitionSeason` (`competitionId`, free-text `label` — competitions here span calendar-year NBA, split-year KPL, and single-event PDC Darts, so no structured year range would fit all of them — `isCurrent`, `@@unique([competitionId, label])`). `Fixture.seasonId`/`StandingRow.seasonId` added **nullable**, matching the existing `homeClubId`/`awayClubId` optional-link precedent from §36 — a real, safe SQLite migration (confirmed clean: additive columns only, zero data loss, all 4,589 existing fixtures and 250 existing standing rows untouched by the migration itself).
- **Backfill** (`prisma/backfill-competition-seasons.js`, safe to re-run): one current `CompetitionSeason` per competition that doesn't have one yet, retroactively assigning every existing null-`seasonId` row to it. Run against the local dev DB: 35 seasons created, 4,589 fixtures and 250 standing rows assigned, zero left orphaned. Generic label ("Current Season") by design — never a guessed real year, since guessing wrong would misrepresent real competition data.
- **`server/lib/seasonResolution.js`**: `getOrCreateCurrentSeason(prisma, competitionId)`, the one function every write path below calls — so "which season does this belong to" is resolved identically everywhere, including for a brand-new competition that's never had a season created for it yet.
- **Every fixture/standings write path updated**: `PUT /:id/standings` (delete-then-recreate now scoped to `{competitionId, seasonId}`, not just `competitionId` — the actual fix that makes history survive), `POST /:id/fixtures`, `POST /:id/fixtures/bulk`, and all 4 sync jobs (`syncLeagues.js`, `syncKenyaCup.js`, `syncTheSportsDB.js`, `syncBallDontLie.js`) — fixtures still upsert on the unchanged `[competitionId, homeTeam, awayTeam, kickoff]` key (kickoff already differentiates seasons naturally; no migration risk there), standings still delete-then-recreate but now season-scoped.
- **Admin** (`public/admin/js/competitions.js`): each competition card shows its current season label and past seasons (view-only), plus a "Start New Season" action (`POST /:id/seasons`) — archives the current season and creates a new one in one transaction. One-way and forward-only by design: there's no "pick a season to edit" control, since editing history isn't a real workflow here, only recording it and moving forward. The existing standings/fixture forms need no other change — they already always write to "whichever season resolves as current."
- **Public** (`public/js/competition.js`, `public/js/scores.js`): `GET /api/competitions/:slug?season=<id>` scopes standings/fixtures to a specific season (defaulting to current); a `<select>` appears on the competition page whenever a competition has more than one season, and switching it does a full reload with the new `?season=` — simpler and more robust than hot-swapping `subnav.js`'s already-rendered tab scope. Every other `fetchCompetitionDetail` caller (club page, sport page, home page tables) is unaffected — the season param is optional and omitted everywhere except this one selector.
- **Verified**: full local flow via curl against dev.db (start a new season, confirm the old season's 8 fixtures/18 standings survive untouched, confirm the new season starts empty, confirm a new fixture/standings write attaches to the new season) — then reverted dev.db to its exact prior state. `e2e/competition-seasons.spec.js` covers the same flow through the real admin UI and public season selector. Full 20-test Playwright suite green before and after.

## 41. Push notifications — the Wave 3 (Personalization) notification foundation (2026-09-16)

The master directive's WAVE 3 is Follows + preferences + a notification foundation + fan voting. Follows (§30) and fan voting (Polls, §33) already existed; this closes the one real remaining gap — deliberately the smallest real, working loop, not a general notification platform (the directive's own words: "Do not create a complicated notification platform unless justified").

- **Schema**: `PushSubscription` (`anonymousId`, `endpoint` unique, `p256dh`/`auth` keys) — keyed to the *same* anonymous device id `Follow` already uses. No new preference model: a device subscribes once, site-wide ("Enable Notifications" in the footer), and what it actually receives is filtered against that device's real `Follow` rows at send time (`server/lib/push.js`'s `sendPushToFollowers(targets, payload)`, deduped by anonymousId so following both a club and its competition doesn't double-send).
- **Server**: `web-push` (VAPID). `server/lib/push.js` no-ops with a warning if `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY` aren't set — same fail-soft convention as this app's other optional integrations. `server/routes/push.js` — `GET /vapid-public-key`, `POST/DELETE /subscribe`, rate-limited like `follows.js`. A dead subscription (404/410 from the push service) is deleted on send rather than retried forever.
- **Client**: `public/js/push.js`, a single button in the shared footer (`public/js/site.js`'s `renderFooter`) — every one of the 15 public pages that load `site.js` now also loads `push.js`. Standard Web Push pattern: fetch the VAPID key, request permission, `pushManager.subscribe()`, POST the subscription. `public/sw.js` gained `push`/`notificationclick` handlers (icon from the real `/brand/favicon/` assets, not invented ones).
- **The one wired trigger**: `PUT /fixtures/:fixtureId` (admin marks a fixture FINISHED with a score) sends to followers of either club or the competition — reuses the exact "newly finished" detection the changelog summary already had. Deliberately **not** wired into the 4 sync jobs yet (API-sourced results) — that needs each job to diff old-vs-new status per fixture before upserting, a real, separate scoping decision, not bundled into this foundation pass. Match reminders (the directive's other example) are deferred the same way — no scheduler exists for it yet.
- **A real bug caught by testing, not review**: the toggle button's own state check was `btn.textContent.includes('on')` — which is true for "🔔 Enable Notifications" too, since the word "Notifications" itself contains the substring "on". Every click silently took the wrong branch. Fixed with an explicit `data-subscribed` attribute instead of text sniffing. Only surfaced because `e2e/push-notifications.spec.js` drove the real button via a real click, not a direct function call.
- **A confirmed, real Playwright/Chromium limitation** (not a bug, spent real time proving this before working around it): Playwright's bundled Chromium test build has no embedded Google API keys, so `pushManager.subscribe()` can never complete for real in this suite, on any context type (tried both the default and a `launchPersistentContext` real-profile context — the second removes Chromium's separate "no Push API in incognito" restriction but not this one). `e2e/push-notifications.spec.js` tests what's actually true here instead: the real client code reaches the real Push API and fails **visibly and recoverably** (a shown error, button returns to a clickable state) rather than hanging or breaking silently. The fixture-result trigger's fail-soft guarantee is tested for real, with a genuinely-undeliverable (but real-shaped) subscription endpoint — no mocking.
- **A pre-existing test-suite fragility, surfaced and fixed, not caused**: the full Playwright suite performs enough independent admin logins across spec files that it started tripping the `loginLimiter` (`max: 10` per 15 minutes, §34) once this session's push test added one more. No test asserts on the limiter's actual threshold, so `E2E_TEST_MODE=true` (set only by `playwright.config.js`'s `webServer` command, same pattern as its `SESSION_SECRET` override) now raises `max` to 1000 for test runs only — production keeps the real cap.
- **iOS note, stated honestly rather than oversold**: Web Push works in Safari only once the site is added to the Home Screen (iOS 16.4+) — a bare Safari tab can't subscribe. Given this audience is mobile-first (most real traffic is phones, not desktop), this materially caps reach until/unless "Add to Home Screen" is promoted somewhere — not fixed here, just not hidden.
- **Verified**: full local round-trip via curl (`vapid-public-key`, subscribe, delete) against dev.db, then confirmed zero `PushSubscription` rows left over. `e2e/push-notifications.spec.js` (2 tests). Full 22-test Playwright suite green.

## 42. Admin Operations — dashboard buckets, scheduled publishing, match reports, real image upload (2026-09-16)

The master directive's WAVE 4 covers 5 things: command dashboards, action queues, nontechnical CMS workflows, correspondence assignment, evidence/verification UX. A survey before starting found the last two — the Correspondent Pipeline (Fixture → Assignment → Correspondent → Submission → Evidence → Verification → Canonical Record → Publication) — already built almost verbatim to the directive on the **Data Platform** side (`Assignment`/`Submission`/`Evidence` models, `my-assignments`/`submissions` admin screens), while Sportscast has none of it. Building a second, competing correspondent system in Sportscast would have directly contradicted the founder's own explicit decision (§10's Roadmap) to shelve the "data venture" phase until the core media operation is solid — so that's deliberately **not** duplicated here; only two small, real gaps in the Data Platform's existing pipeline were closed (separate write-up, that repo's own BLUEPRINT/commit history). This entry covers the genuinely-Sportscast-shaped remainder: dashboard + action queues + nontechnical CMS.

- **Dashboard** (`public/admin/js/dashboard.js`): three new "what needs attention today" buckets alongside the existing Articles/Monitoring/Submissions/failed-sources cards — **Fixtures** (today's matches across every sport, via a new `GET /api/fixtures/today`; a match that's kicked off but has no result yet gets its own alert card, same treatment as a failing sync source), **Production** (draft `VIDEO_POST` articles — episodes still in edit), **Publishing** (scheduled stories, soonest first). All computed from data already being fetched — only `/fixtures/today` is a new query.
- **Scheduled publishing**: `Article.status` gains a third value, `SCHEDULED`, alongside a nullable `scheduledAt`. `server/jobs/publishScheduled.js` (new cron, every 2 minutes) flips a due article to `PUBLISHED` and sets `publishedAt` to the *scheduled* moment, not whenever the cron happened to run — a story scheduled for 09:00 still says it went live at 09:00. Validated server-side (`SCHEDULED` requires a real `scheduledAt`) and client-side (the admin form won't submit without one). The admin `status` `<select>` gained the option rather than being redesigned into buttons — kept consistent with this app's own existing status-dropdown convention (Fixture status uses the same pattern) rather than inventing a new UI language for one field.
- **Match reports**: `Article.fixtureId` — a direct, single-fixture link, distinct from the existing club/competition *tags* (broad: "this story covers KPL") — this is precise: "this story is the report on this exact game." Admin gets a search-as-you-type fixture picker (`GET /api/fixtures/search`, admin-only — there are 4,700+ fixtures, far too many for a `<select>`), mirroring the existing canonical-event-search widget's UX. `public/js/match.js`'s Match Hub page shows a directly-linked report in its own pinned "Match Report" section, above the existing tag-based "Related Coverage" heuristic (which still exists as the fallback for matches with no direct report).
- **Real image upload** (`server/routes/uploads.js`, `multer` + `sharp`): `coverImageUrl` was a raw URL text field only — no upload existed anywhere in this codebase. `POST /api/uploads/image` (admin-only) accepts JPEG/PNG/WebP, applies EXIF-orientation correction, resizes to max 1600px wide, re-encodes as JPEG q82, and stores it — automating the same discipline §35 established by hand for one photo. **Storage location decision, checked before assuming**: `railway ssh ... df -h /data` showed 421MB actually free on the production volume (the "478MB/500MB" `railway status` reports is a different, higher-level metric — confirmed by directly measuring, not guessed) — safe to store on the same persistent volume sessions/the DB already live on, avoiding a new S3/R2 dependency and its credentials/cost for what a modest newsroom needs. Video stays YouTube-ID-based, deliberately not extended to native file upload — a materially bigger, costlier undertaking (transcoding, storage, bandwidth) this pass doesn't take on.
- **Verified**: real end-to-end curl round-trips locally (upload → resize confirmed via `sharp.metadata()` → serve → 401 when unauthenticated → 400 on a non-image file), scheduled publish tested against the real cron (not simulated — `node server/jobs/publishScheduled.js` run directly, confirmed `publishedAt` matches the original `scheduledAt`), fixture attach/search/match-report display tested via real API calls. `e2e/admin-wave4.spec.js` (5 tests, including a real photo built with `sharp` uploaded through the actual file input, and the real cron picking up a real scheduled article — `PUBLISH_SCHEDULED_CRON` tightened to every 10s for this suite only, `playwright.config.js`, same pattern as `E2E_TEST_MODE`). Full 27-test Playwright suite green.

## 43. Content Transformation Engine — real transcription/clipping/captioning pipeline (2026-09-16)

The master directive's WAVE 5 (§22 Media/Show Architecture, §23 Content Transformation Engine): SHOW → SEASON → EPISODE → MEDIA ASSETS → CLIPS → TRANSCRIPT, and an AI-assisted pipeline turning a long-form recording into a transcript, suggested clips, and social copy. This directly overlaps a roadmap item (§10, Phase 1: "AI-assisted clipping pipeline — Transcribe existing show recordings (Whisper), have an LLM flag the 3–5 most shareable moments, cut with ffmpeg, burn in captions") the founder had explicitly **shelved** until the core media operation was fully stable — Phase 0 itself wasn't even complete (three shows still had mismatched placeholder photos). Surfaced this tension directly rather than quietly building or quietly skipping it; the founder's explicit call was to build the real thing now, reversing that shelving deliberately, not by drift.

- **Episode architecture** (`prisma/schema.prisma`): `Episode.host`/`.guest` (free-text strings) replaced with real relations — `hosts Author[]` (hosts are always this newsroom's own bylined staff) and `EpisodeGuest` (required `name` — many real guests, e.g. a federation official, will never have a `Player` row — with an optional best-effort `playerId` link, same "string is the source of truth, the link is additive" precedent as `Fixture.homeClubId`). No backfill needed: confirmed all 4 pre-existing episodes had null host/guest. New `Clip`, `SocialAsset`, `EpisodeSponsor` models (the last deliberately separate from the existing Club-scoped `Sponsor` — a shirt sponsor and a podcast sponsor are different relationships that happen to share a shape).
- **Transcription** (`server/lib/whisper.js`): OpenAI's hosted Whisper API — this app has no GPU and Railway's container isn't sized to run a local speech model. Whisper's 25MB single-upload cap is handled for real: `server/lib/ffmpeg.js` extracts a compressed mono 64kbps audio track first, and `contentEngine.transcribeRecording` chunks it (20-min segments) and re-aligns each chunk's timestamps onto the recording's real timeline when the compressed track is still too large for one call.
- **AI clip suggestion & quote generation** (`server/lib/contentEngine.js`, Claude via the existing `@anthropic-ai/sdk` — same explicit-`apiKey` pattern as `runMonitoringEnrich.js`, its implicit env-var pickup doesn't reliably work in every context this runs in): Claude is given the real, timestamped transcript segments and picks WHICH moments are shareable and WHY — it never writes caption text itself. A clip's actual captions (`buildSrtForRange`) are always a literal slice of the real Whisper segments, re-timed to the clip's own 0-based timeline. Quote-card generation (`suggestSocialAssets`) goes further: every returned quote is verified as a real (normalized) substring of the transcript before it's trusted — one that doesn't verify is silently dropped, never saved. This is the directive's "never let AI invent quotations" rule enforced as code, not just a system-prompt instruction — confirmed directly: a fabricated quote fed through `normalizeForMatch` was rejected, a real one passed.
- **Human review is a real state, not just a policy**: `Clip.status` — `SUGGESTED` (AI's idea, unreviewed) → human `APPROVED`/`REJECTED` → `RENDERED` once actually cut. A `MANUAL` clip (an editor picked the range directly) skips straight to `APPROVED` — nothing to review. Only an `APPROVED` clip can be rendered.
- **Rendering** (`server/lib/ffmpeg.js`'s `cutClip`): cuts the exact range from the ORIGINAL raw recording (video preserved, not just audio) and burns in captions when available. **A real environment gap, caught by testing, not assumed away**: caption burn-in needs ffmpeg built with `libass` — confirmed directly that Homebrew's plain `ffmpeg` formula doesn't include it (`ffmpeg-full` does), and there's no way to be certain any given `ffmpeg` build does either. `cutClip` treats a caption-burn failure as recoverable: it falls back to a plain (uncaptioned) render rather than failing the whole clip.
- **Getting `ffmpeg` onto the production container — a real, first-attempt-wrong infra lesson, not assumed away**: first shipped a `nixpacks.toml` requesting `ffmpeg` via `[phases.setup] nixPkgs`, the standard way to add a system package to a Nixpacks build — deployed clean, but `ffmpeg` still wasn't on `PATH`. `railway ssh` confirmed why: this service's production container is **Debian (`trixie`), not a Nix store** (`ls /nix` → no such directory) — it's built with **Railpack**, Railway's newer default builder, which silently ignores `nixpacks.toml` since that's a different tool's config format entirely. Verified the actual fix against Railpack's real source (`core/config/config.go` on GitHub, not the docs site, which 404'd for the specific page) before shipping it a second time: `railpack.json` at the repo root, `deploy.aptPackages: ["ffmpeg"]` — the field that ships an apt package in the **final runtime image**, distinct from top-level `buildAptPackages` (build-step only, wouldn't help here since this app's own server invokes `ffmpeg` at request time, not at build time). `nixpacks.toml` removed rather than left alongside as dead, misleading config.
- **Raw recording storage — a real disk-capacity decision, not assumed**: `railway ssh ... df -h /data` (same check as §42's image-upload decision) showed real headroom, but a 45-60min show recording is hundreds of MB — nothing like a compressed photo. Kept off the persistent volume as long as possible: `contentEngine.transcribeRecording` does its audio-extraction/chunking work in the OS temp dir, not `/data`; the raw recording itself is written to `prisma/raw-recordings/` (same persistent volume, but a distinct directory from Wave 4's `uploads/`) only because a later, human-paced clip-approval step still needs the original file. Two independent safety nets bound this: an explicit "Delete Raw Recording" admin action, and `server/jobs/cleanupRawRecordings.js` (new cron, hourly) that force-deletes anything older than 48h regardless of whether an editor remembered.
- **A real, non-obvious bug found only by testing the actual click, not the API in isolation**: an early version of the render e2e test named its test clip "E2E Rendered Clip" — its own status-wait assertion (`toContainText('Rendered')`) matched the clip's TITLE the instant it appeared in the DOM, long before the real (fast, but non-instant) render had actually finished, making a still-broken render look like it passed instantly. Fixed by renaming the test clip and polling the real API for `status === 'RENDERED'` instead of DOM text — the render pipeline itself was correct the whole time; the test was lying about it.
- **Two credentials this environment genuinely doesn't have** — flagged honestly, not glossed over: `OPENAI_API_KEY` (Whisper transcription) and `ANTHROPIC_API_KEY` (AI clip suggestions, quote generation — the pre-existing monitoring engine has this identical gap, confirmed via its own "ANTHROPIC_API_KEY not set" log line). Both fail soft with a clear message; nothing pretends to succeed. Everything not gated behind those two keys — the full schema, hosts/guests, manual clip creation, real ffmpeg cutting, real caption burn-in (verified with a locally-installed `ffmpeg-full` binary via `FFMPEG_PATH`), the caption-burn fallback, sponsors, and the fail-soft upload/status flow — was verified for real, not just reviewed.
- **Verified**: real end-to-end local testing throughout (a synthetic ffmpeg-generated test video, a real 5-second cut confirmed via `ffprobe` duration, real burned-in captions confirmed by extracting and visually inspecting a frame from the rendered output). `e2e/content-engine.spec.js` (4 tests: hosts/guests with a real linked player, a genuinely ffmpeg-rendered clip with correct duration, episode sponsors, and the transcription-failure path preserving the raw file for manual use). Full 31-test Playwright suite green.
- **Deliberately not built**: native video hosting/transcoding as a general capability (clips are the one video-file case this app now handles, scoped tightly to the render pipeline — not a general-purpose video CMS), and any UI surfacing SUGGESTED clips/quotes to the public before a human has reviewed them (`GET /api/articles/:slug` only ever includes clips — no status filter needed today since nothing public reads Clip directly yet; if a public "Clips" section is added later, it must filter to `RENDERED` only, matching Match Hub's own "Match Report" pattern of only ever showing human-confirmed content).

## 44. Wave 7 — API / App Readiness: documented public API, a real notification/event bus, sitemap, fan accounts (2026-09-16)

The master directive's WAVE 7 (six bullets: clean public APIs, authentication, deep links, notification contracts, PWA, client-ready data access). A survey against all six first found PWA and client-ready data access already real (installable manifest, working service worker, every public GET route already usable by a hypothetical client) — the four genuine gaps were: this repo never had any API documentation at all, the one push-notification pathway was hardcoded to a single trigger, no sitemap/robots.txt existed, and there was no registered-user tier, only anonymous device tokens.

- **`API.md`** (repo root, new): this site's first-ever API reference — versioning stance (bare `/api/*`, no prefix yet, since exactly one consumer exists today; a real `/api/v2` only if a breaking change is ever genuinely needed), the public/gated split resource-by-resource, and the two independent auth tracks. Mirrors the sibling Data Platform repo's own `docs/API.md`, which this repo never had an equivalent of.
- **Notification/event bus** (`server/lib/events.js`, new): generalizes the one hardcoded push call (a fixture result → its followers, previously inline in `competitions.js`) into `notifyArticlePublished(articleId)` — derives `{entityType, entitySlug}` targets from an Article's own `sport`/`competitions`/`clubs`/`players` relations and calls the already-generic `sendPushToFollowers`. Wired into all three moments an Article actually becomes `PUBLISHED` (immediate publish, edit-triggers-publish, the scheduled-publish cron) — an Article carrying an Episode gets a "New episode" title instead of "New story," since Episode has no status of its own (it publishes exactly when its Article does). Deliberately no new stored event-log table — see `DECISION_LOG.md`'s Wave 7 entry for why that would have been speculative here, unlike the Data Platform's `DomainEvent`.
- **`server/routes/sitemap.js`** (new): `GET /sitemap.xml` + `GET /robots.txt`, neither of which existed before. Lists the static top-level pages plus every `PUBLISHED` Article, `isActive` Show, Competition, Club, and Sport — the same five entities `seoPages.js` already gives real metadata to. Player/Fixture excluded (high-cardinality, same reasoning the Data Platform's own sitemap uses). Introduces this repo's first `SITE_URL` env var (defaulted to the real production origin) — no prior absolute-URL convention existed anywhere here.
- **Fan accounts** (`FanAccount` model, `server/routes/fanAuth.js`, `public/account.html`/`public/js/account.js`, all new): the REGISTERED tier of the directive's ANONYMOUS/REGISTERED/STAFF/API_CLIENT model. Email + password, `bcrypt`-hashed, session stored at a distinct `req.session.fan` (never collides with staff's `req.session.user` in the same browser). Registering claims the device's existing `anonymousId` onto the account rather than migrating any Follow/PushSubscription rows — both models already key on `anonymousId`, so nothing needs to move. **No password-reset flow** — this stack has no email-sending service, confirmed before building anything; a reset link with nowhere to send its email would be worse than none. See `DECISION_LOG.md`'s Wave 7 entry for the full reasoning on both calls, made explicitly, not assumed.
- **Verified**: 8 new Playwright tests (`e2e/push-notifications.spec.js`'s new describe block, `e2e/sitemap.spec.js`, `e2e/fan-account.spec.js`) plus the existing suite, all passing — 39 tests total, up from 32. The fan-account UI test drives the real `/account.html` form through a real browser, not just the API.
- **Deliberately not built**: cross-device follow sync for a logged-in fan account (would need `public/js/follows.js` to stop trusting `localStorage` once signed in — a real client-side redesign), email verification, and a `/v2` API prefix (nothing depends on `/v1` breaking yet).
